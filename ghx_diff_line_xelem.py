"""
This script does not work well in the cases a .ghx is changed widely.
The difflib algorithm seems to misunderstand wide changes
(e.g. the cases object xms element are added or removed)
as sets of small changes (e.g. changes of component class changes).
2021-11-22
"""
...
class Diff_Mgr:
    def __init__(self, unified_diff, bef_xml, bef_line_xelem_map, aft_xml, aft_line_xelem_map,):
        ...
        diff = diff.split("\n@@")
        print("Diff_Mgr: len(diff): ", len(diff))
        ...
        print("Diff_Mgr: len(diff_chunk_list): ", len(self.diff_chunk_list))
    ...
    def fetch_changed_xelements(self):
        ...
        removed_xelements = [t for t in removed_xelements if not t.xpath("@name=\"Object\"")]
        for t in removed_xelements:
            print(t, t.attrib)
        ...
        added_xelements = [t for t in added_xelements if not t.xpath("@name=\"Object\"")]
        ...
        print("removed_instance_guids xelements: ", len(removed_instance_guids))
...
def indicate_changed_objects_as_group(tgt_xml, removed_guids, modified_guids, added_guids):
    ...
    if len(modified_guids):
        desc = generate_group_obj_xelem(obj_count, "modified_cmps", modified_guids, "255;127;255;255")
        def_objs_chunks_xelem.insert(obj_count, et.fromstring(desc))
        obj_count += 1
    if len(added_guids):
